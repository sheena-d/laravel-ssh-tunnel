...
use STS\Tunneler\Console\TunnelerCommandActivate;
use STS\Tunneler\Console\TunnelerCommandClose;
...
class TunnelerServiceProvider extends ServiceProvider{
    ...
    public function register()
    {
        ...
        $this->app->singleton('command.tunneler.activate',
            function ($app) {
                return new TunnelerCommandActivate();
            }
        );

        $this->commands('command.tunneler.activate');

        $this->app->singleton('command.tunneler.close',
            function ($app) {
                return new TunnelerCommandClose();
            }
        );

        $this->commands('command.tunneler.close');
    }
    ...
    {
        return ['command.tunneler.activate', 'command.tunneler.close'];
    }
